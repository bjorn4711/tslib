/*
 * tslib/plugins/egalax-raw.c
 *
 * Inspired from input-raw.c
 * 2009.05 Fred Salabartan
 *
 * Modified handling of input keys and changed the name to egalax
 * 2011.05 Björn Carlson, bjorm.carlson@l-w.com
 *
...
#define USB_PRODUCT_ID	0x7269
//#define USB_VERSION_ID	0x0112
#define USB_VERSION_ID	0x0210
...
static int ts_egalax_check_fd (struct tslib_egalax *i)
{
  ...
  } else if (infos.bustype != 3 || infos.vendor != USB_VENDOR_ID || 
		infos.product != USB_PRODUCT_ID || infos.version != USB_VERSION_ID) {
    fprintf (stderr, "tslib: this is not an eGalax touchscreen (3,0x%X,1,0x%X)\n"
             "Your device: bus=%d, vendor=0x%X, product=0x%X, version=0x%X\n",
		USB_PRODUCT_ID, USB_VERSION_ID,
...
static int ts_egalax_read (struct tslib_module_info *inf,
                          struct ts_sample *samp, int nr)
{
  struct tslib_egalax *i = (struct tslib_egalax *)inf;
  ...
  if (i->sane_fd == SANE_INIT)
    i->sane_fd = ts_egalax_check_fd(i);
  ...
  while (total < nr) {
    ...
    case EV_ABS:
      switch (ev.code) {
      case ABS_Z:		i->current_x = ev.value; break;
      case ABS_RX:		i->current_y = ev.value; break;
      }
      break;
    }
  }
...
static int ts_egalax_fini (struct tslib_module_info *inf)
{
  struct tslib_egalax *i = (struct tslib_egalax *)inf;
  struct tsdev *ts = inf->dev;
...
static const struct tslib_ops __ts_egalax_ops = {
  .read	= ts_egalax_read,
  .fini	= ts_egalax_fini,
};
...
TSAPI struct tslib_module_info *egalax_mod_init (struct tsdev *dev, const char *params)
{
  struct tslib_egalax *i;

  i = malloc (sizeof (struct tslib_egalax));
  if (i == NULL)
    return NULL;

  i->module.ops = &__ts_egalax_ops;
  i->current_x = 0;
  i->current_y = 0;
  i->current_pressure = 0;
  i->sane_fd = SANE_INIT;
...
#ifndef TSLIB_STATIC_EGALAX_MODULE
	TSLIB_MODULE_INIT(egalax_mod_init);
#endif
